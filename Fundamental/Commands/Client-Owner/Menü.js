...
let Database = Custom.find({})
if(Database && Database.length >= 1) {
  for (let index = 0; index < Database.length; index++) {
      client.on("interactionCreate", async (interaction) => { 
          let menu = interaction.customId
          const member = await client.guilds.cache.get(settings.sunucuId).members.fetch(interaction.member.user.id)
          if (!member) return;
                      ...
                      if(i.customId == "kur") {
                          ...
                          if(kurulcak) {
                              ...
                              message.channel.send({content: `${kurulcak.Text}`, components: [listMenu]}).then(async (oluşturuldu) => {
                                  ...
                                  collector.on('collect', async (i) => {
                                      const member = await client.guilds.cache.get(settings.sunucuId).members.fetch(i.user.id)
                                      if (!member) return;
