...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (sec == "setup") {
        ...
        if (message.guild.members.cache.some(member => conf.sahip.some(sahip => member === sahip)) || message.member.permissions.has("ADMINISTRATOR") || message.author.id === message.guild.owner.id) {
            ...
            if(!message.member.permissions.has("8") && !server.MUTEAuthorized.some(rol => message.member.roles.cache.has(rol))  &&!server.UstYetkiliRol.some(rol => message.member.roles.cache.has(rol))) return message.react(`${client.emojis.cache.find(x => x.name === "wex_cancel")}`)    
        ...
        let muteSorumlusu = data.MUTEAuthorized;
                    ...
                    if (muteSorumlusu.length >= 1 && client.channels.cache.get(muteLogKanal) && muteLimit >= 1) {
                        ...
                        .setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({dynamic:true}) }).setDescription(`${client.emojis.cache.find(res => res.name === "wex_mute")} ${target} (**${target.id}**) kullanıcısı ${message.author} tarafından **${reason}** sebebiyle geçiçi olarak susturuldu. (Ceza Numarası: \`#${cezaID+1}\`)`)
