...
let userChannel  = require("../models/weeklyvoicechannel");
...
client.on("voiceChannelSwitch", async (member, oldChannel, newChannel) => {
 ...
 let duration = Date.now() - data.start;
  ...
  await voiceUser.findOneAndUpdate({ guildID: member.guild.id, userID: member.id }, { $inc: { topStat: duration, dailyStat: duration, weeklyStat: duration } }, { upsert: true });
	await userChannel.updateOne({ guildID: member.guild.id, userID: member.id, channelID: data.channel, parentID: newChannel.parentId }, { $inc: { channelData: duration, parentData: duration } }, { upsert: true });
...
client.on("voiceChannelLeave", async (member, channel) => {
  ...
  addVoiceStat(member.id, data.channel, duration, duration / 1000, data.parent);
  await voiceUser.findOneAndUpdate({ guildID: member.guild.id, userID: member.id }, { $inc: { topStat: duration, dailyStat: duration, weeklyStat: duration } }, { upsert: true });
  await userChannel.updateOne({ guildID: member.guild.id, userID: member.id, channelID: data.channel, parentID: data.parent }, { $inc: { channelData: duration, parentData: duration } }, { upsert: true });
