...
module.exports = function (client) {
  ...
  client.checkPermission = async (bots, id, type) => {
    let member = client.guilds.cache.get(client.ayarlar.sunucuId).members.cache.get(id);
    ...
    if (!res) {
        ...
        if (client.ayarlar.sahip.some(uye => uye == member?member.id:false) || res.Full.some(uye => uye == member?member.id:false  || member ? member.roles.cache.has(uye) : false)) {
            return true;
        }
        if (type == "full") {
      ...
      client.kanalbackup = async () => {
        const guild = client.guilds.cache.get(client.ayarlar.sunucuId);
          ...
          client.allPermissionOpen = async () => {
            const Koru = require('../models/Koruma');
        ...
        client.cezaVer = async (test, kisiID, tur) => {
            ...
            let MEMBER = test.guilds.cache.get(client.ayarlar.sunucuId).members.cache.get(kisiID)
            if (!MEMBER) return;
            ...
            if (tur == "jail") return MEMBER.roles.cache.has(data.BOOST) ? MEMBER.roles.set([data.BOOST, data.JAIL]) : MEMBER.roles.set([data.JAIL]).catch(err => console.log(err));
