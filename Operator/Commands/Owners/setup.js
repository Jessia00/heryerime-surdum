...
if (err) console.log(err)
            ...
            if (["topseskategori", "topseskat"].some(y => y === sec)) {
                let select = message.mentions.channels.first();
                if (!select) return message.react(client.emojis.cache.find(res => res.name === "wex_iptal"));
                data.TopSesKategori = select.id, await data.save(), message.react(client.emojis.cache.find(res => res.name === "wex_tik"));
            };
            if (["topsesmesaj", "topsesmsj"].some(y => y === sec)) {
                let select = args[1];
                if (!select) return message.react(client.emojis.cache.find(res => res.name === "wex_iptal"));
                data.TopSesMesaj = select.id, await data.save(), message.react(client.emojis.cache.find(res => res.name === "wex_tik"));
            };

            
            if (["topmesajkategori", "topmesajkat"].some(y => y === sec)) {
                let select = message.mentions.channels.first();
                if (!select) return message.react(client.emojis.cache.find(res => res.name === "wex_iptal"));
                data.TopChatKategori = select.id, await data.save(), message.react(client.emojis.cache.find(res => res.name === "wex_tik"));
            };
            if (["ship-kanal"].some(y => y === sec)) {
                let select = message.mentions.channels.first();
                if (!select) return message.react(client.emojis.cache.find(res => res.name === "wex_iptal"));
                data.ShipChannel = select.id, await data.save(), message.react(client.emojis.cache.find(res => res.name === "wex_tik"));
            };

            if (["topmesajmesaj", "topmesajmsj"].some(y => y === sec)) {
                let select = args[0];
                if (!select) return message.react(client.emojis.cache.find(res => res.name === "wex_iptal"));
                data.TopChatMesaj = select.id, await data.save(), message.react(client.emojis.cache.find(res => res.name === "wex_tik"));
            };
            ...
            if (["sorumluluklar"].some(y => y === sec)) {
                ...
                data.sorumluluklar = select, await data.save(), message.react(client.emojis.cache.find(res => res.name === "wex_tik"));
            };
            if (["onlineofflineperms"].some(y => y === sec)) {
                let select;
                if (message.mentions.roles.size >= 1) {
                    select = message.mentions.roles.map(r => r.id);
                } else {
            ...
            if (["warnauth"].some(y => y === sec)) {
                ...
                data.WARNAuthorized = select, await data.save(), message.react(client.emojis.cache.find(res => res.name === "wex_tik"));
            };
            ...
            if (["etkinlik-katılımcısı", "ekatılımcı"].some(y => y === sec)) {
                let select = message.mentions.roles.first();
                if (!select) return message.react(client.emojis.cache.find(res => res.name === "wex_iptal"));
                data.EtkinlikKatilimcisi = select.id, await data.save(), message.react(client.emojis.cache.find(res => res.name === "wex_tik"));
            };
            if (["çekiliş-katılımcısı", "çkatılımcı"].some(y => y === sec)) {
                let select = message.mentions.roles.first();
                if (!select) return message.react(client.emojis.cache.find(res => res.name === "wex_iptal"));
                data.CekilisKatilimcisi = select.id, await data.save(), message.react(client.emojis.cache.find(res => res.name === "wex_tik"));
            };
            ...
            if (["guardBanLimit"].some(y => y === sec)) {
                let select = Number(args[1])
                if (!select) return message.react(client.emojis.cache.find(res => res.name === "wex_iptal"));
                data.GuardBanLimit = select, data.save(), message.react(client.emojis.cache.find(res => res.name === "wex_tik"));
            };
            if (["guardKickLimit"].some(y => y === sec)) {
                let select = Number(args[1])
                if (!select) return message.react(client.emojis.cache.find(res => res.name === "wex_iptal"));
                data.GuardKickLimit = select, data.save(), message.react(client.emojis.cache.find(res => res.name === "wex_tik"));
            };
            if (["guardRolLimit"].some(y => y === sec)) {
                let select = Number(args[1])
                if (!select) return message.react(client.emojis.cache.find(res => res.name === "wex_iptal"));
                data.GuardRolLimit = select, data.save(), message.react(client.emojis.cache.find(res => res.name === "wex_tik"));
            };
            if (["guardKanalLimit"].some(y => y === sec)) {
                let select = Number(args[1])
                if (!select) return message.react(client.emojis.cache.find(res => res.name === "wex_iptal"));
                data.GuardRolLimit = select, data.save(), message.react(client.emojis.cache.find(res => res.name === "wex_tik"));
            };
