...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (sunucuData.GKV.includes(message.author.id) || client.ayarlar.sahip.includes(message.author.id)) {
        ...
        if (["oluştur"].includes(sec)) {
            ...
            newData.save();
            message.channel.send({embeds: [new Discord.EmbedBuilder().setFooter({ text: conf.footer }).setAuthor({name: client.user.username, iconURL: client.user.avatarURL()}).setDescription(`\`${komutAd}\` adlı komut başarılı bir şekilde oluşturuldu.
...
Verecek Kişiler: ${args2[2] ? kişiler : "`Kişi eklenmedi.`"}`)]});
        ...
        if (["bak"].includes(sec)) {
            ...
            let mesaj = new Discord.EmbedBuilder().setFooter({ text: conf.footer }).setAuthor({name: client.user.username, iconURL: client.user.avatarURL()})
...
`)
 message.channel.send({embeds: [mesaj]})
        ...
        if (!sec) {
            ...
            message.channel.send({embeds: [new Discord.EmbedBuilder().setFooter({ text: conf.footer }).setAuthor({name: client.user.username, iconURL: client.user.avatarURL()}).setDescription(`Aşağıdaki özel komutlar'ı daha detaylı bir şekilde görmek için aşağıdaki komutu yazınız.\n\`.özelkomut bak <komutAdı>\`\n\n${göster}`)]});
        };
    } else return;
};
exports.conf = {
